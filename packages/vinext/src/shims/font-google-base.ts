...
const _CLASS_COUNTER_KEY = Symbol.for("vinext.font.classCounter");
...
const _INJECTED_ROOT_VARIABLES_KEY = Symbol.for("vinext.font.injectedRootVariables");
...
(_g[_CLASS_COUNTER_KEY] as { value: number }) ??= { value: 0 };
const classCounterRef = _g[_CLASS_COUNTER_KEY] as { value: number };
...
const injectedRootVariables = ((_g[_INJECTED_ROOT_VARIABLES_KEY] as Set<string>) ??=
  new Set<string>());
...
export function createFontLoader(family: string): FontLoader {
  return function fontLoader(options: FontOptions & { _selfHostedCSS?: string } = {}): FontResult {
    const id = classCounterRef.value++;
